...
def train_one_epoch(model, train_batch, criterion, train_X, train_Y, train_loss_arr, device):
    model.train()
    train_loss = 0.0
...
def val_one_epoch(model, val_batch, criterion, val_X, val_Y, val_loss_arr, device):
    model.eval()
    with torch.no_grad():
        val_loss = 0.0
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        print(f"epoch:{epoch}")
        train_one_epoch(model, train_batch, criterion, train_df_X, train_df_Y, train_loss_arr, CFG.device)
        val_one_epoch(model, train_batch, criterion, train_df_X, train_df_Y, train_loss_arr, CFG.device)
