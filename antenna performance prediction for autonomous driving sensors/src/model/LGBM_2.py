...
class CFG:
    ...
    trainPath = dataPath+'raw/new_categorical_features_train.csv'

    testPath = dataPath+'raw/new_categorical_features_test.csv'
...
def data_preprosessing(train_df, test_df):
    ...
    train_N = train_df.filter(regex='N')
    train_X = train_df.filter(regex='X') # Input : X Featrue
    train_X = pd.concat([train_X, train_N], axis=1)
    
    test_N = test_df.filter(regex='N')
    test_X = test_df.filter(regex='X')
    test_X = pd.concat([test_X, test_N], axis=1)
    ...
    test_final_X = test_X.copy()
    train_X, test_X, train_Y, test_Y = train_test_split(train_X, train_Y, test_size=0.2, random_state=42)
    train_X, val_X, train_Y, val_Y = train_test_split(train_X, train_Y, test_size=0.2, random_state=42)
    

    return test_final_X, train_X, train_Y, val_X, val_Y, test_X, test_Y

seed_everything(CFG.seed) # Seed 고정

train_df = pd.read_csv(CFG.trainPath) # train.csv load
test_df = pd.read_csv(CFG.testPath) # test.csv load


test_final_X, train_X, train_Y, val_X, val_Y, test_X, test_Y = data_preprosessing(train_df, test_df)
...
for i in range(1, 15):
    ...
    settings = {
        ...
        "log_file_name": (y_number+'.log'),  # flaml log file
    ...
    submit_sample.to_csv('submit_sample_LightGBM.csv', index=False)
    submit_sample = pd.read_csv('submit_sample_LightGBM.csv')

    # 제출 csv 추출
    y_pred_final = automl.predict(test_final_X)
    ...
    submit = pd.read_csv('submit_LightGBM.csv')
